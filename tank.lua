...
local TANK_BULLET_DAMAGE = 350
local TANK_FORWARD_SPEED = 5
local TANK_REVERSE_SPEED = -3
local TANK_MAX_PITCH_RAD = math.rad(18)
local TANK_TILT_LOOKAHEAD = 3.3
...
local function find_surface_y_at(pos)
	for dy = 2, -4, -1 do
		local base = {x = pos.x, y = pos.y + dy, z = pos.z}
		local above = {x = base.x, y = base.y + 1, z = base.z}
		if node_is_walkable(base) and not node_is_walkable(above) then
			return base.y + 1
		end
	end
	return pos.y
...
local function get_tank_target_pitch(self, yaw)
	...
	local forward = { x = math.cos(yaw + math.pi / 2), y = 0, z = math.sin(yaw + math.pi / 2) }
	local sample_dist = TANK_TILT_LOOKAHEAD
	local front_pos = vector.add(pos, vector.multiply(forward, sample_dist))
	local back_pos = vector.subtract(pos, vector.multiply(forward, sample_dist))
	local front_y = find_surface_y_at(front_pos)
	local back_y = find_surface_y_at(back_pos)
	local rise = math.max(-1.0, math.min(1.0, front_y - back_y))
	local pitch = math.atan2(rise, sample_dist * 2)
	if pitch > TANK_MAX_PITCH_RAD then
		pitch = TANK_MAX_PITCH_RAD
	elseif pitch < -TANK_MAX_PITCH_RAD then
		pitch = -TANK_MAX_PITCH_RAD
	end
	return pitch
end

local function apply_tank_rotation(self, yaw, pitch)
	if not self or not self.object then
		return
	end
	pitch = pitch or 0
...
minetest.register_entity("dogwar:tank", {
	...
	on_activate = function(self, staticdata)
 	 ...
 	 on_death = function(self, killer)
            ...
            -- detach and (optionally) kill any driver; don't reposition, let the
            -- normal death/respawn logic handle where the player comes back.
            if self.driver then
                ...
                -- if still alive, kill them so they will respawn normally
                pcall(function()
	...
	on_step = function(self, dtime)
		...
		-- if the driver has died or disconnected, detach them now so the
		-- vehicle doesn't keep moving with an invalid reference.
		if self.driver then
			if (not self.driver:is_player()) or self.driver:get_hp() <= 0 then
				-- simulate an exit like on_rightclick
				local ply = self.driver
				pcall(function()
					if ply.get_attach and ply:get_attach() then
						ply:set_detach()
		...
		if vel.y == 0 then
			...
			if turned then
				...
				apply_tank_rotation(self, yaw, self.pitch or 0)
			end
			if ctrl.up then
				self.object:set_velocity({x=math.cos(yaw+math.pi/2)*TANK_FORWARD_SPEED, y=vel.y, z=math.sin(yaw+math.pi/2)*TANK_FORWARD_SPEED})
				anim = {{x=0, y=19}, 30, 0}
				moved = true
			end
			...
			if ctrl.down then
				self.object:set_velocity({x=math.cos(yaw+math.pi/2)*TANK_REVERSE_SPEED, y=vel.y, z=math.sin(yaw+math.pi/2)*TANK_REVERSE_SPEED})
				anim = {{x=20, y=39}, 15, 0}
				moved = true
			end
			...
			if moved then
				target_pitch = get_tank_target_pitch(self, yaw)
			end
			local current_pitch = self.pitch or 0
			local smooth = math.min(1, dtime * 6)
			self.pitch = current_pitch + (target_pitch - current_pitch) * smooth
			apply_tank_rotation(self, yaw, self.pitch)
		end
...
-- utility to detach players riding a dogwar tank; reused by callbacks below
local function _dogwar_detach_player(player)
    if not player then return end
    local attach = player:get_attach()
    if attach and attach:get_luaentity() then
        local ent = attach:get_luaentity()
        if ent and ent.name == "dogwar:tank" then
...
-- clear the driver when a player dies or leaves to avoid orphaned controls
minetest.register_on_dieplayer(_dogwar_detach_player)
minetest.register_on_leaveplayer(_dogwar_detach_player)
